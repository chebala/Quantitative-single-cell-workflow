...
This normalization can be done with Z-scores [(x-median)/MAD] which I used for correlation analysis(not shown here).
...
Using Z- scores, the expression distribution of each marker under different treatment condition can be visualized though ridge plots or violin plots with an integrated box plots (Here, I used log2(1+x) normalized scores).
...
As the analyses are performed at the single-cell level, pseudoreplication may influence statistical significance. Reporting effect sizes alongside p-values and incorporating biological replicates where possible is recommended.
...
This is a graphical summary of the workflow from preparing the cytospin samples to analyzing the mIF data. 
